...
public:
	typedef std::function<void(std::shared_ptr<SignalingNotifyInfo>)> SignalingNotifyCallback;	/**< 单条通知回调 */
	typedef std::function<void(std::list<std::shared_ptr<SignalingNotifyInfo>>)> SignalingNotifyListCallback;	/**< 多条通知回调 */
	...
	* @param[in] cb 通知回调，返回SignalingNotifyInfo的子类
	* @return void 无返回值
	*/
	...
	* @param[in] cb 通知回调，返回SignalingNotifyInfo子类的list
	* @return void 无返回值
	*/
	...
	*/
	...
	/** @fn static void Join(SignalingJoinParam param, SignalingOptCallback cb)
	* @brief 独立信令 查询频道接口
	* @param[in] param 查询频道的传入参数
	* @param[in] cb 结果回调见nim_signaling_helper.h，返回SignalingQueryChannelInfoResParam
	* @return void 无返回值\n
	* 错误码：		200:成功;
	*				  0:接口服务异常;
	*				408:超时;
