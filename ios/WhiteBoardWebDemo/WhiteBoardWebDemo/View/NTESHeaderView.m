...
- (void)refreshWithRoomId:(NSString *)roomId userName:(NSString *)userName {
    ...
    [self.pasteButton mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(self);
        make.left.equalTo(self.roomIDLabel.mas_right).offset(5);
...
- (void)onTouchExitRoom:(id)sender {
    if (self.delegate && [self.delegate respondsToSelector:@selector(didTriggerExitAction)]) {
        [self.delegate didTriggerExitAction];
    }
}
